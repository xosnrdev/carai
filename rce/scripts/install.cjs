...
(async () => {
  ...
  for await (const pkg of packages) {
    ...
    try {
      ...
      console.log(
        `Package installation for ${pkg.name} done, reading the configuration file`
      );
      ...
      const environment =
        config["environment"]?.reduce((acc, curr) => {
      ...
      console.log(
        `Environment for ${pkg.name}: ${JSON.stringify(environment, null, 2)}`
      );
      ...
      if (config.compiled) {
        ...
        if (testResult.trim() !== "Hello world!") {
          throw new Error(
            `Test file failed for package ${
              pkg.name
            }. Expecting "Hello world!", got "${testResult.trim()}"`
          );
        }
      } else {
        const testResult = await execute(
      ...
      console.error(`An error occurred while installing ${pkg.name}: ${error.message}`);
      process.exit(1);
    }
  }
})();
